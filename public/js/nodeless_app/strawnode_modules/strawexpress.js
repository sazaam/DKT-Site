	...
	return Pkg.write('org.libspark.straw', function(){
		...
		var Hierarchy = Type.define({
			...
			launchDeep:function launchDeep(path){
				...
				// trace('********************')
				var p = new CommandQueue(hh.formulate(path)) ;
				hh.command = p ;
				var current = hh.currentStep ;
				
				if(Type.is(current, Unique)) {
					trace('in here') ; hh.command.execute() ; // cast Unique in that case
				} else{
					var foc_clear ;
			...
			closeCommand:function closeCommand(path, c, nofuture){
				var hh = c.context ;
				var st = hh.getDeep(path) ;
				var st_close ;
				var st_closing ;
		...
		var AddressHierarchy = Type.define({
			...
			statics:{
				...
				isReady:function isReady(){
					...
					return trace(base == AddressHierarchy.parameters.base) ;
				},
